...
def analyze_song_workflow(
    ...
    resolved_analysis_mode = (analysis_mode or "core").strip().lower()
    if resolved_analysis_mode not in {"core", "full"}:
        raise ValueError("analysis_mode must be either core or full.")
    ...
    overall_audio_info: dict[str, Any] = {"audio": "skipped", "reason": "core_mode"}
    overall_raw = ""
    if resolved_analysis_mode == "full":
    ...
    raw_steps: dict[str, Any] = {
        "workflow": [
            "overall_analysis" if resolved_analysis_mode == "full" else "overall_analysis_skipped",
            "songformer_structure",
            "lyric_asr" if resolved_analysis_mode == "full" else "lyric_asr_skipped",
            *chord_workflow_steps,
            "group_sections",
        ],
            ...
            "steps": [
                {
                    ...
                    "status": "done" if resolved_analysis_mode == "full" else "skipped",
                },
                {
                ...
                },
                {
                    ...
                    "status": (
                        "skipped"
                        if resolved_analysis_mode == "core"
